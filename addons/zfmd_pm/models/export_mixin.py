...
class ZfmdContractExport(models.Model):
    ...
    def _export_columns(self):
        return [
            ...
            ("initial_fee", "初装费（万元）", 14),
            ("service_fee", "预测服务费（万元）", 16),
            ("amount_total", "合同总额（万元）", 16),
            ("amount_untaxed", "不含税合同金额（万元）", 20),
            ...
            ("third_party_interface_fee", "第三方接口费（万元）", 18),
...
class ZfmdProjectStartExport(models.Model):
    ...
    def _export_columns(self):
        return [
            ...
            ("estimated_contract_amount", "预计合同金额（万元）", 18),
            ("estimated_cost_amount", "预计成本（万元）", 16),
            ("actual_contract_amount", "实际合同金额（万元）", 18),
            ("delivery_department", "交付部门", 14),
            ("project_manager", "项目经理", 14),
...
class ZfmdServiceRecordExport(models.Model):
    ...
    def _export_columns(self):
        return [
            ...
            ("expired_months", "超期时间（月）", 12),
            ("is_overdue", "是否超期", 12),
            ("expected_contract_amount", "预计签订服务合同金额（万元）", 22),
...
class ZfmdInvoiceRecordExport(models.Model):
    ...
    def _export_columns(self):
        return [
            ...
            ("sale_manager", "销售经理", 14),
            ("sale_contact", "销售联系人", 14),
            ("contract_amount", "合同金额（万元）", 18),
            ("invoice_amount", "开票金额（万元）", 18),
            ("tax_rate", "税率", 10),
            ("amount_untaxed", "不含税金额（万元）", 18),
            ("promised_payment_date", "承诺回款日期", 16),
            ("promised_payment_amount", "承诺回款金额（万元）", 20),
            ("actual_payment_date", "实际回款日期", 16),
            ("actual_payment_amount", "实际回款（万元）", 18),
...
class ZfmdPaymentRecordExport(models.Model):
    ...
    def _export_columns(self):
        return [
            ...
            ("contract_amount", "合同金额（万元）", 18),
            ("payment_type", "类型", 12),
            ("bill_amount", "汇票回款（万元）", 18),
            ("cash_amount", "现金回款（万元）", 18),
            ("amount_total", "回款金额（万元）", 18),
...
class ZfmdReceivablePlanExport(models.Model):
    ...
    def _export_columns(self):
        return [
            ...
            ("is_summary_line", "是否汇总行", 12),
            ("receivable_amount", "应收金额（万元）", 18),
